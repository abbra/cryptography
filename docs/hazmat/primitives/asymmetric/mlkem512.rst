...
.. currentmodule:: cryptography.hazmat.primitives.asymmetric.mlkem512


ML-KEM-512 is a post-quantum key encapsulation mechanism (KEM) based on
module lattices, standardized in `FIPS 203`_. It provides NIST security level 1
(equivalent to AES-128) and is suitable for bandwidth-constrained environments
and IoT devices. ML-KEM-512 allows one party to securely encapsulate a shared
secret that can only be decapsulated by the holder of the corresponding
private key.

For most applications, :doc:`mlkem768` is recommended as it provides a better
security margin.
...
.. doctest::
    ...
    >>> from cryptography.hazmat.primitives.asymmetric.mlkem512 import MlKem512PrivateKey
    >>> from cryptography.hazmat.primitives.kdf.hkdf import HKDF
    >>> from cryptography.hazmat.primitives import serialization
    >>> # Server generates a key pair
    >>> server_private_key = MlKem512PrivateKey.generate()
...
.. class:: MlKem512PrivateKey
    ...
    .. classmethod:: generate()
        ...
        Generate an ML-KEM-512 private (decapsulation) key.

        :returns: :class:`MlKem512PrivateKey`

        :raises cryptography.exceptions.UnsupportedAlgorithm: If ML-KEM-512 is
            not supported by the OpenSSL version ``cryptography`` is using.

    .. classmethod:: from_private_bytes(data)

        A class method for loading an ML-KEM-512 key encoded as
        :attr:`~cryptography.hazmat.primitives.serialization.Encoding.Raw`.

        :param bytes data: 1632 byte private key.

        :returns: :class:`MlKem512PrivateKey`

        :raises ValueError: If the private key is not 1632 bytes.
        ...
        .. doctest::
            ...
            >>> from cryptography.hazmat.primitives import serialization
            >>> from cryptography.hazmat.primitives.asymmetric import mlkem512
            >>> private_key = mlkem512.MlKem512PrivateKey.generate()
            >>> private_bytes = private_key.private_bytes(
            ...     encoding=serialization.Encoding.Raw,
            ...     format=serialization.PrivateFormat.Raw,
            ...     encryption_algorithm=serialization.NoEncryption()
            ... )
            >>> loaded_private_key = mlkem512.MlKem512PrivateKey.from_private_bytes(private_bytes)

    .. method:: public_key()

        :returns: :class:`MlKem512PublicKey`

    .. method:: decapsulate(ciphertext)

        Decapsulates the ciphertext to recover the shared secret.

        :param ciphertext: The 768-byte ciphertext from encapsulation.
        :type ciphertext: :term:`bytes-like`

        :returns bytes: A 32-byte shared secret.

        :raises ValueError: If the ciphertext is not 768 bytes.
    ...
    .. method:: private_bytes_raw()
        ...
        :return bytes: 1632-byte raw private key.
...
.. class:: MlKem512PublicKey
    ...
    .. classmethod:: from_public_bytes(data)
        ...
        :param bytes data: 800 byte public (encapsulation) key.

        :returns: :class:`MlKem512PublicKey`

        :raises ValueError: If the public key is not 800 bytes.
        ...
        .. doctest::
            ...
            ... )
            >>> loaded_public_key = mlkem512.MlKem512PublicKey.from_public_bytes(public_bytes)
    ...
    .. method:: encapsulate()
        ...
        :returns tuple[bytes, bytes]: A tuple of (ciphertext, shared_secret).
            The ciphertext is 768 bytes and the shared_secret is 32 bytes.
    ...
    .. method:: public_bytes_raw()
        ...
        :return bytes: 800-byte raw public key.
