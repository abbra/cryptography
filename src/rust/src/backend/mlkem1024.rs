...
#[pyo3::pyclass(
    frozen,
    module = "cryptography.hazmat.bindings._rust.openssl.mlkem1024"
)]
...
#[pyo3::pyfunction]
fn generate_key() -> CryptographyResult<MlKem1024PrivateKey> {
    Ok(MlKem1024PrivateKey {
        pkey: openssl::pkey::PKey::generate_ml_kem(openssl::pkey_ml_kem::Variant::MlKem1024)?,
    })
}
...
#[pyo3::pyfunction]
fn from_private_bytes(data: CffiBuf<'_>) -> pyo3::PyResult<MlKem1024PrivateKey> {
    let pkey = openssl::pkey::PKey::private_key_from_raw_bytes_ex(data.as_bytes(), "ML-KEM-1024")
        .map_err(|_| {
        pyo3::exceptions::PyValueError::new_err("Invalid ML-KEM-1024 private key")
    })?;
    Ok(MlKem1024PrivateKey { pkey })
}

#[pyo3::pyfunction]
fn from_public_bytes(data: &[u8]) -> pyo3::PyResult<MlKem1024PublicKey> {
    let pkey = openssl::pkey::PKey::public_key_from_raw_bytes_ex(data, "ML-KEM-1024")
        .map_err(|_| pyo3::exceptions::PyValueError::new_err("Invalid ML-KEM-1024 public key"))?;
...
impl MlKem1024PrivateKey {
    fn decapsulate<'p>(
        ...
        if ciphertext.as_bytes().len() != 1568 {
    ...
    fn public_key(&self) -> CryptographyResult<MlKem1024PublicKey> {
        ...
        Ok(MlKem1024PublicKey {
            pkey: openssl::pkey::PKey::public_key_from_raw_bytes_ex(&raw_bytes, "ML-KEM-1024")?,
        })
    }
...
pub(crate) mod mlkem1024 {
    #[pymodule_export]
    use super::{
        from_private_bytes, from_public_bytes, generate_key, MlKem1024PrivateKey,
        MlKem1024PublicKey,
    };
}
