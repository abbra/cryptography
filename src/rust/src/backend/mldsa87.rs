...
#[pyo3::pyfunction]
fn generate_key() -> CryptographyResult<MlDsa87PrivateKey> {
    Ok(MlDsa87PrivateKey {
        pkey: openssl::pkey::PKey::generate_ml_dsa(openssl::pkey_ml_dsa::Variant::MlDsa87)?,
    })
}
...
#[pyo3::pyfunction]
fn from_private_bytes(data: CffiBuf<'_>) -> pyo3::PyResult<MlDsa87PrivateKey> {
    let pkey = openssl::pkey::PKey::private_key_from_raw_bytes_ex(data.as_bytes(), "ML-DSA-87")
        .map_err(|_| pyo3::exceptions::PyValueError::new_err("Invalid ML-DSA-87 private key"))?;
    Ok(MlDsa87PrivateKey { pkey })
}

#[pyo3::pyfunction]
fn from_public_bytes(data: &[u8]) -> pyo3::PyResult<MlDsa87PublicKey> {
    let pkey = openssl::pkey::PKey::public_key_from_raw_bytes_ex(data, "ML-DSA-87")
        .map_err(|_| pyo3::exceptions::PyValueError::new_err("Invalid ML-DSA-87 public key"))?;
...
impl MlDsa87PrivateKey {
    ...
    fn public_key(&self) -> CryptographyResult<MlDsa87PublicKey> {
        ...
        Ok(MlDsa87PublicKey {
            pkey: openssl::pkey::PKey::public_key_from_raw_bytes_ex(&raw_bytes, "ML-DSA-87")?,
        })
    }
...
pub(crate) mod mldsa87 {
    #[pymodule_export]
    use super::{
        from_private_bytes, from_public_bytes, generate_key, MlDsa87PrivateKey, MlDsa87PublicKey,
    };
}
