...
fn from_private_bytes(data: CffiBuf<'_>) -> pyo3::PyResult<MlDsa65PrivateKey> {
    let pkey = openssl::pkey::PKey::private_key_from_raw_bytes_ex(data.as_bytes(), "ML-DSA-65")
...
pub(crate) mod mldsa65 {
    #[pymodule_export]
    use super::{
        from_private_bytes, from_public_bytes, generate_key, MlDsa65PrivateKey, MlDsa65PublicKey,
    };
}
