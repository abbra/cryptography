...
fn private_key_from_pkey<'p>(
    ...
    #[cfg(CRYPTOGRAPHY_OPENSSL_350_OR_GREATER)]
    {
            ...
            .is_some()
        {
            ...
            .is_some()
        {
            return Ok(crate::backend::mlkem1024::private_key_from_pkey(pkey)
                .into_pyobject(py)?
                .into_any());
        }
    }
...
fn public_key_from_pkey<'p>(
    ...
    // Check for ML-DSA keys using the ml_dsa() method
    // Check for ML-KEM keys using the ml_kem() method
    #[cfg(CRYPTOGRAPHY_OPENSSL_350_OR_GREATER)]
    {
        if pkey
            .ml_dsa(openssl::pkey_ml_dsa::Variant::MlDsa44)?
            ...
            .is_some()
        {
            ...
            .is_some()
        {
            return Ok(crate::backend::mlkem1024::public_key_from_pkey(pkey)
                .into_pyobject(py)?
                .into_any());
        }
    }
