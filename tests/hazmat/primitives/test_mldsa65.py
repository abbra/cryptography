...
@pytest.mark.supported(
    only_if=lambda backend: not backend.mldsa65_supported(),
    skip_message="Requires OpenSSL without ML-DSA-65 support",
)
def test_mldsa65_unsupported(backend):
    ...
    with raises_unsupported_algorithm(
        ...
        MlDsa65PrivateKey.from_private_bytes(b"0" * 4032)
...
@pytest.mark.supported(
    only_if=lambda backend: backend.mldsa65_supported(),
    skip_message="Requires OpenSSL with ML-DSA-65 support",
)
class TestMlDsa65Signing:
    ...
    def test_pub_priv_bytes_raw(self, backend):
        ...
        # ML-DSA-65 key sizes
        assert len(private_raw) == 4032
        assert len(public_raw) == 1952

        # Verify we can load the keys back
        loaded_private = MlDsa65PrivateKey.from_private_bytes(private_raw)
        loaded_public = MlDsa65PublicKey.from_public_bytes(public_raw)
    ...
    def test_load_public_bytes(self, backend):
        public_key = MlDsa65PrivateKey.generate().public_key()
        public_bytes = public_key.public_bytes(
            serialization.Encoding.Raw, serialization.PublicFormat.Raw
        )
        public_key2 = MlDsa65PublicKey.from_public_bytes(public_bytes)
    ...
    def test_invalid_type_private_bytes(self, backend):
        with pytest.raises(TypeError):
            MlDsa65PrivateKey.from_private_bytes(
                object()  # type: ignore[arg-type]
            )

    def test_invalid_length_from_public_bytes(self, backend):
        with pytest.raises(ValueError):
            MlDsa65PublicKey.from_public_bytes(b"a" * 1951)
        with pytest.raises(ValueError):
            MlDsa65PublicKey.from_public_bytes(b"a" * 1953)

    def test_invalid_length_from_private_bytes(self, backend):
        with pytest.raises(ValueError):
            MlDsa65PrivateKey.from_private_bytes(b"a" * 4031)
        with pytest.raises(ValueError):
            MlDsa65PrivateKey.from_private_bytes(b"a" * 4033)
...
def test_public_key_equality(backend):
    ...
    key2_priv = MlDsa65PrivateKey.generate()
...
def test_public_key_copy(backend):
    key1 = MlDsa65PrivateKey.generate().public_key()
    key2 = copy.copy(key1)
    assert key1 == key2
