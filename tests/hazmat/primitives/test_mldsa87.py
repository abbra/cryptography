...
def test_mldsa87_unsupported(backend):
    ...
    with raises_unsupported_algorithm(
        ...
        MlDsa87PrivateKey.from_seed_bytes(b"0" * 4032)
...
class TestMlDsa87Signing:
    def test_sign_verify(self, backend):
        ...
        # ML-DSA-87 signatures are 4595 bytes according
        # to FIPS 204 section 4 table 2
        # Note: OpenSSL currently produces 4627-byte signatures for ML-DSA-87
        # instead of the 4595 bytes specified in FIPS 204 Table 2
    ...
    def test_pub_priv_bytes_raw(self, backend):
        ...
        MlDsa87PrivateKey.from_seed_bytes(seed)
    ...
    def test_invalid_length_from_private_bytes(self, backend):
        with pytest.raises(ValueError):
            MlDsa87PrivateKey.from_seed_bytes(b"a" * 31)
        with pytest.raises(ValueError):
            MlDsa87PrivateKey.from_seed_bytes(b"a" * 33)
...
def test_rfc9881_seed_only_format(backend):
    """
    RFC 9881 Section 6 defines ML-DSA-87-PrivateKey as a CHOICE with three
    formats:
