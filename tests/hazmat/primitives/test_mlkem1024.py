...
from cryptography.hazmat.primitives.asymmetric.mlkem1024 import (
    MlKem1024PrivateKey,
    MlKem1024PublicKey,
)
...
@pytest.mark.supported(
    only_if=lambda backend: not backend.mlkem1024_supported(),
    skip_message="Requires OpenSSL without ML-KEM-1024 support",
)
def test_mlkem1024_unsupported(backend):
    ...
    with raises_unsupported_algorithm(
        ...
        MlKem1024PrivateKey.from_private_bytes(b"0" * 3168)
...
@pytest.mark.supported(
    ...
    skip_message="Requires OpenSSL with ML-KEM-1024 support",
)
class TestMlKem1024:
    def test_encapsulate_decapsulate(self, backend):
        private_key = MlKem1024PrivateKey.generate()
    ...
    def test_pub_priv_bytes_raw(self, backend):
        ...
        assert len(private_raw) == 3168
        assert len(public_raw) == 1568

        # Verify we can load the keys back
        loaded_private = MlKem1024PrivateKey.from_private_bytes(private_raw)
        loaded_public = MlKem1024PublicKey.from_public_bytes(public_raw)
    ...
    def test_load_public_bytes(self, backend):
        public_key = MlKem1024PrivateKey.generate().public_key()
        public_bytes = public_key.public_bytes(
            serialization.Encoding.Raw, serialization.PublicFormat.Raw
        )
        public_key2 = MlKem1024PublicKey.from_public_bytes(public_bytes)
    ...
    def test_public_bytes_der(self, backend):
        ...
        assert isinstance(loaded_public, MlKem1024PublicKey)
    ...
    def test_private_bytes_der(self, backend):
        ...
        assert isinstance(loaded_private, MlKem1024PrivateKey)
    ...
    def test_invalid_private_bytes(self, backend):
        with pytest.raises(ValueError):
            MlKem1024PrivateKey.from_private_bytes(b"0" * 100)

    def test_invalid_public_bytes(self, backend):
        with pytest.raises(ValueError):
            MlKem1024PublicKey.from_public_bytes(b"0" * 100)
    ...
    def test_equality(self, backend):
        private_key1 = MlKem1024PrivateKey.generate()
        private_key2 = MlKem1024PrivateKey.generate()
